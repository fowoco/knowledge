...
required_documents_manual_raw.csv 파일은 사람이 직접 작성하는 파일이며
required_id만 비워둔 형태입니다.
"""
...
def _load_rows(input_path: Path) -> list[dict[str, str]]:
    ...
    with input_path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in reader:
            ...
            cleaned_row = {_clean(k): _clean(v) for k, v in row.items() if k is not None}
...
def _norm_is_essential(value: str | None, *, line_no: int) -> str:
    ...
    v = _clean(value).upper()
    if v not in {"Y", "N"}:
        raise ValueError(f"{line_no}행: is_essential 값은 Y/N이어야 합니다 (입력값: '{value}')")
    return v
...
def generate(
    ...
    *,
    ...
    for line_no, row in enumerate(rows, start=2):  # 헤더가 1행이므로 데이터는 2행부터
        ...
        requirement_id = stable_id("REQ", application_name, document_name, source_industry_text)
        ...
        candidate_row = {
            ...
            "applicable_scope": _norm_scope(row.get("applicable_scope", ""), source_industry_text),
            ...
            "is_essential": _norm_is_essential(row.get("is_essential"), line_no=line_no),
